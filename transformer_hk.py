...
@dataclasses.dataclass
class CrossSelfAttnTransformer(hk.Module):
		'''
		cross-attention transformer, with self-attention added to query
		Note that in MultiHeadAttention, the query, key, value will be reshaped via linear projection
		query -> Q [t, key_size]
		key   -> K [T, key_size]
		value -> V [T, value_size]
		...
		def __call__(
						...
						key: jnp.ndarray,  # [T, key_size]
						value: jnp.ndarray,  # [T, value_size]
						self_mask=None,  # [1, t, t]
						cross_mask=None,  # [1, t, T]
						final_norm=True
		) -> jnp.ndarray:  # [t, D_o]
				...
				for i in range(self.num_layers):
						...
						self_attn_block = hk.MultiHeadAttention(
								num_heads=self.num_heads,
								key_size=self.QK_size,
								w_init=initializer,
								value_size=self.V_size,
						...
						this_query = self_attn_block(query=query_norm, key=query_norm, value=query_norm, mask=self_mask)
						query_norm = layer_norm(this_query + query_norm)
						...
						this_query = attn_block(query=query_norm, key=key_norm, value=value_norm, mask=cross_mask)
						query_norm = layer_norm(this_query + query_norm)
