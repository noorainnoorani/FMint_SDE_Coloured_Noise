...
def build_matrices_from_data_shape(data_shape, config, compact, mode, caption_len, shot_num_min, return_shape_list = False):
    ...
    # build_basic_mask is the figure 3 in icon-lm paper, without caption yet
...
class IconGPTModel(nn.Module):
	...
	def setup(self):
		...
		self.pre_projection = nn.Dense(self.config['transformer']['model_dim'], name="pre_projection") # 256

		if self.config['index_mode'] == 'learn':
		# trainable positional embedding
			self.func_pos_embedding = nn.Embed((self.config['demo_max_num']) * 3, self.config['transformer']['model_dim'], name="func_pos_embedding") # 6 * 3 -> 256
	...
	def basic_forward(self, data, mode, index_matrix, basic_mask, caption_len, shot_num_min):
		...
		############ build cond, qoi_kv, qoi_k sequence
		...
		if caption_len > 0:
            ...
            # concatenating caption and data embedding
		...
		if caption_len > 0:
			# embedding_mask = jnp.atleast_1d(data.embedding_mask)
			# mask = jnp.atleast_1d(mask)
			# mask = jnp.concatenate([embedding_mask, mask], axis = 0)
...
def build_network_fn(data, key, config, return_model = False, compact = True, print_model = True):
	...
	data_shape = tree.tree_map(lambda x: x.shape, data) # shape of simplified data
	...
	params = model.init(rngs, data) # parameters of the model from the '.init' method
	if print_model:
