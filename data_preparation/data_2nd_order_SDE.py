...
def double_well_fn(init, control, ts, dt, params, k, step_fn, key):
	...
	# Align coarse and fine states with selected times ts[::k]
	selected_times = ts[::k]
	L = selected_times.shape[0]

	init_stack = jnp.stack([init[0], init[1]], axis=0)          # (2,)
	coarse_first = init_stack[:, None]                          # (2,1)
	coarse_rest = jnp.stack([large_traj[0][:-1],
	                         large_traj[1][:-1]], axis=0)       # (2,L-1)
	coarse_at_selected = jnp.concatenate([coarse_first, coarse_rest], axis=1)  # (2,L)

	fine_indices = jnp.arange(1, L) * k - 1
	fine_rest = jnp.stack([fine_traj[0][fine_indices],
	                       fine_traj[1][fine_indices]], axis=0)               # (2,L-1)
	fine_first = init_stack[:, None]
	fine_at_selected = jnp.concatenate([fine_first, fine_rest], axis=1)       # (2,L)

	errors = fine_at_selected - coarse_at_selected
	large_states = coarse_at_selected

	return selected_times, large_states, large_noise, errors
...
def coupled_doublewell_fn(init, control, ts, dt, params, k, step_fn, key):
	...
	init_stack = jnp.stack([init[0], init[1]], axis=0)
	coarse_first = init_stack[:, None]
	coarse_rest = jnp.stack([large_traj[0][:-1],
	                         large_traj[1][:-1]], axis=0)
	coarse_at_selected = jnp.concatenate([coarse_first, coarse_rest], axis=1)

	fine_indices = jnp.arange(1, L) * k - 1
	fine_rest = jnp.stack([fine_traj[0][fine_indices],
	                       fine_traj[1][fine_indices]], axis=0)
	fine_first = init_stack[:, None]
	fine_at_selected = jnp.concatenate([fine_first, fine_rest], axis=1)
...
def generate_one_dyn(key, ode_batch_fn, dt, length, num,  init_range, params,k,N = 1, eqn = "duffing"):
	...
	if eqn == 'perturbed_nonlinearoscillator' or eqn == 'periodic_nonlinearoscillator':
		selected_times, selected_u, dW, errors = ode_batch_fn(
	...
	return selected_times[..., None], selected_u[..., None], dW, errors[..., None]
...
if __name__ == "__main__":
	...
	# selected_times, selected_u, dW, errors = generate_one_dyn(key = next(rng), ode_batch_fn = double_well_batch_fn
	# 						, dt=1e-5, length = 200, num =1,  init_range = [(-1,1),(-1,1)], params = [alpha,beta]
	# 						,k = 10)
	# selected_times, selected_u, dW, errors = generate_one_dyn(key = next(rng), ode_batch_fn = coupled_doublewell_batch_fn
	# 						, dt=1e-5, length = 200, num =1,  init_range = [(-1,1),(-1,1)], params = [alpha,beta]
	# 						,k = 10)
	...
	# selected_times, selected_u, dW, errors = generate_one_dyn(key = next(rng), ode_batch_fn = mueller_overdamped_batch_fn
	...
	# selected_times, selected_u, dW, errors = generate_one_dyn(key = next(rng), ode_batch_fn = duffing_langevin_batch_fn
	...
	selected_times, selected_u, dW, errors = generate_one_dyn(key = next(rng), ode_batch_fn = perturbed_nonlinearoscillator_batch_fn
	...
	# selected_times, selected_u, dW, errors = generate_one_dyn(key = next(rng), ode_batch_fn = periodic_nonlinearoscillator_batch_fn
