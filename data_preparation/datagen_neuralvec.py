...
def generate_lotka_volterra(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step):
	...
	'''
	rng = hk.PRNGSequence(jax.random.PRNGKey(seed))
	alpha_s = jax.random.uniform(next(rng), (eqns,), minval = 0.1, maxval = 1.0)
	beta_s = jax.random.uniform(next(rng), (eqns,), minval = 0.01, maxval = 0.1)
	gamma_s = jax.random.uniform(next(rng), (eqns,), minval = 0.1, maxval = 1.0)
	delta_s = jax.random.uniform(next(rng), (eqns,), minval = 0.01, maxval = 0.1)
...
def generate_harmonic_oscillator(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step):
	'''d^x/dt^2 = -\omega^2 * x
...
def generate_duff_equa(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step):
...
def generate_van_der_pol(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step):
	...
	'''
	rng = hk.PRNGSequence(jax.random.PRNGKey(seed))
	eps_s = jax.random.uniform(next(rng), (eqns,), minval = 0.1, maxval = 5.0)
...
def generate_damped_harmonic_oscillator(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step):
	...
	'''
	rng = hk.PRNGSequence(jax.random.PRNGKey(seed))
	zeta_s = jax.random.uniform(next(rng), (eqns,), minval = 0.1, maxval = 2.0)
	omega_s = jax.random.uniform(next(rng), (eqns,), minval = 0.5, maxval = 5.0)
...
def generate_driven_damped_pendulum(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step):
...
if __name__ == "__main__":
	...
	flags.DEFINE_integer('num', 10, 'number of systems in each equation') # fixed paramter
