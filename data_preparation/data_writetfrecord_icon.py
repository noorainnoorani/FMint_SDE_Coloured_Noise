...
def write_ICON_1st_order_tfrecord(name, eqn_type, all_params, all_eqn_captions, all_ts, all_ys,alpha):
	num = all_ts[0].shape[0]

	filename = "{}_{}_{}.tfrecord".format(name, eqn_type, alpha)
	...
	with tf.io.TFRecordWriter(filename) as writer:
		for params, ts_expand, selected_u in zip(all_params, all_ts, all_ys):
			...
			values = jnp.ones_like(ts_expand)
			cond_v_c = jnp.pad(values, ((0,0),(0,0),(0,2)), mode = 'constant', constant_values = 0.0)
			
			cond_v_i = jnp.pad(selected_u[:,0:1,:],((0,0),(0,0),(0,2)),mode = 'constant',constant_values = 0.0)  # table 1, u(0)
			cond_v = jnp.concatenate([cond_v_c, cond_v_i], axis = 1)
			
			selected_u = jnp.pad(selected_u,((0,0),(0,0),(0,2)),mode = 'constant',constant_values = 0.0)
			...
			count += 1
			if np.sum(selected_u) != np.nan:
				s_element= serialize_element(equation = equation_name, caption = None, 
							cond_k = cond_k, cond_v = cond_v, qoi_k = ts_expand, qoi_v = selected_u,
							count = count)
...
def write_ICON_2nd_order_tfrecord(name, eqn_type, all_params, all_eqn_captions, all_ts, all_ys,alpha):
	num = all_ts[0].shape[0]
	...
	with tf.io.TFRecordWriter(filename) as writer:
		for params, ts_expand, selected_u in zip(all_params, all_ts, all_ys):
			equation_name = "{}_params_{}_nv_step_{}".format(eqn_type, params,alpha)
			print('ts_expand',ts_expand.shape) # (100, 199, 1)
			...
			cond_v_i = jnp.pad(selected_u[:,0:1,:],((0,0),(0,0),(0,1)),mode = 'constant',constant_values = 0.0)  # table 1, u(0)
			cond_v = jnp.concatenate([cond_v_c, cond_v_i], axis = 1)

			selected_u = jnp.pad(selected_u,((0,0),(0,0),(0,1)),mode = 'constant',constant_values = 0.0)
...
def write_ICON_3rd_order_tfrecord(name, eqn_type, all_params, all_eqn_captions, all_ts, all_ys,alpha):
	num = all_ts[0].shape[0]
	...
	with tf.io.TFRecordWriter(filename) as writer:
		for params, ts_expand, selected_u in zip(all_params, all_ts, all_ys):
			...
			cond_v_i = selected_u[:,0:1,:] # table 1, u(0)
			cond_v = jnp.concatenate([cond_v_c, cond_v_i], axis = 1)
	...
	if problem_type == "forward":
		...
		with tf.io.TFRecordWriter(filename) as writer:
			for params, ts_expand, control, traj in zip(all_params, all_ts, all_cs, all_us):
				...
				cond_k_i = einshape('i->jki', jnp.array([0.0,1.0]), j = num, k = 1)
				cond_k = jnp.concatenate([cond_k_c, cond_k_i], axis = 1)
				cond_v_c = control[:,:-1,:] # table 1, c(t1),c(t2),...,c(tn_j-1)
				cond_v_i = traj[:,0:1,:] # table 1, u(0)
				cond_v = jnp.concatenate([cond_v_c, cond_v_i], axis = 1)
				count += 1
