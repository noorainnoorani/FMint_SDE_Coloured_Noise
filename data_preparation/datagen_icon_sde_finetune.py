...
def generate_ornstein_uhlenbeck(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step,num_repeat, testname, testquests):
	...
	# 'test'
	datawrite.write_ICON_SDE_1st_order_tfrecord(name = testname, eqn_type = "ornstein_uhlenbeck", 
				all_params = all_params, all_eqn_captions = all_eqn_captions,
				all_ts = all_ts, all_ys = all_ys,alpha=nv_step, repeat=num_repeat)
...
def generate_double_well(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step,num_repeat, testname, testquests):
	...
	# name is 'train' or 'test'
	datawrite.write_ICON_SDE_2nd_order_tfrecord(name = testname, eqn_type = "double_well", 
				all_params = all_params, all_eqn_captions = all_eqn_captions,
			all_ts = all_ts, all_ys = all_ys,alpha=nv_step, repeat=num_repeat)
...
def generate_coupled_doublewell(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step,num_repeat, testname, testquests):
	...
	# name is 'train' or 'test'
	datawrite.write_ICON_SDE_2nd_order_tfrecord(name = testname, eqn_type = "coupled_doublewell", 
				all_params = all_params, all_eqn_captions = all_eqn_captions,
			all_ts = all_ts, all_ys = all_ys,alpha=nv_step, repeat=num_repeat)
...
def generate_mueller_overdamped(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step,num_repeat, testname, testquests):
	...
	# name is 'train' or 'test'
	datawrite.write_ICON_SDE_2nd_order_tfrecord(name = testname, eqn_type = "mueller_overdamped", 
				all_params = all_params, all_eqn_captions = all_eqn_captions,
			all_ts = all_ts, all_ys = all_ys,alpha=nv_step, repeat=num_repeat)
...
def generate_duffing_langevin(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step,num_repeat, testname, testquests):
	...
	# name is 'train' or 'test'
	datawrite.write_ICON_SDE_2nd_order_tfrecord(name = testname, eqn_type = "duffing_langevin", 
				all_params = all_params, all_eqn_captions = all_eqn_captions,
			all_ts = all_ts, all_ys = all_ys,alpha=nv_step, repeat=num_repeat)
...
def generate_perturbed_nonlinearoscillator(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step,num_repeat, testname, testquests):
	...
	# name is 'train' or 'test'
	datawrite.write_ICON_SDE_2nd_order_tfrecord(name = testname, eqn_type = "perturbed_nonlinearoscillator", 
				all_params = all_params, all_eqn_captions = all_eqn_captions,
			all_ts = all_ts, all_ys = all_ys,alpha=nv_step, repeat=num_repeat)
...
def generate_periodic_nonlinearoscillator(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step,num_repeat, testname, testquests):
	...
	# name is 'train' or 'test'
	datawrite.write_ICON_SDE_2nd_order_tfrecord(name = testname, eqn_type = "periodic_nonlinearoscillator", 
				all_params = all_params, all_eqn_captions = all_eqn_captions,
			all_ts = all_ts, all_ys = all_ys,alpha=nv_step, repeat=num_repeat)
...
def generate_GMB(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step,num_repeat, testname, testquests):
	...
	# name is 'train' or 'test'
	datawrite.write_ICON_SDE_1st_order_tfrecord(name = testname, eqn_type = "geombrownian_motion", 
				all_params = all_params, all_eqn_captions = all_eqn_captions,
				all_ts = all_ts, all_ys = all_ys,alpha=nv_step, repeat=num_repeat)
...
def generate_inhomogeneous_ornsteinuhlenbeck(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step,num_repeat, testname, testquests):
	...
	# name is 'train' or 'test'
	datawrite.write_ICON_SDE_1st_order_tfrecord(name = testname, eqn_type = "inhomogeneous_ornsteinuhlenbeck", 
				all_params = all_params, all_eqn_captions = all_eqn_captions,
				all_ts = all_ts, all_ys = all_ys,alpha=nv_step, repeat=num_repeat)
...
def generate_fluxgate_sensor(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step,num_repeat, testname, testquests):
	...
	for i, (c, lambda_, epsilon,omega) in enumerate(zip(c_k, lambda_k, epsilon_k, omega_k)):
		for j in range(testquests):
			...
			utils.print_dot(i)
	# name is 'train' or 'test'
	datawrite.write_ICON_SDE_3rd_order_tfrecord(name = testname, eqn_type = "fluxgate_sensor", 
				all_params = all_params, all_eqn_captions = all_eqn_captions,
			all_ts = all_ts, all_ys = all_ys,alpha=nv_step, repeat=num_repeat)
...
def generate_stochastic_lorenz(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step,num_repeat, testname, testquests):
	...
	# name is 'train' or 'test'
	datawrite.write_ICON_SDE_3rd_order_tfrecord(name = testname, eqn_type = "stochastic_lorenz", 
				all_params = all_params, all_eqn_captions = all_eqn_captions,
			all_ts = all_ts, all_ys = all_ys,alpha=nv_step, repeat=num_repeat)
...
def generate_predator_prey(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step,num_repeat, testname, testquests):
	...
	sigma3_k = jnp.full_like(s_k, 0.168)
	...
	# name is 'train' or 'test'
	datawrite.write_ICON_SDE_3rd_order_tfrecord(name = testname, eqn_type = 'predator_prey', 
				all_params = all_params, all_eqn_captions = all_eqn_captions,
			all_ts = all_ts, all_ys = all_ys,alpha=nv_step, repeat=num_repeat)
...
def generate_predator_prey2(seed, eqns, quests, length, dt, num, caption_mode, name,nv_step,num_repeat, testname, testquests):
	...
	for i, (r, a, s, b, K, g, D, v1, v2, sigma1, sigma2, sigma3, sigma4) in enumerate(zip(r_k, a_k, s_k, b_k, K_k, g_k, D_k, v1_k, v2_k, sigma1_k, sigma2_k, sigma3_k, sigma4_k)):
		for j in range(quests):
			ts_expand, selected_u, dW = generate_valid_batch(rng, ode_batch_fn = ode_3.predator_prey2_batch_fn, 
									dt = dt, length = length, num = num, init_range = [(0, 0.8),(0,0.8),(0,0.8)],
	...
	# name is 'train' or 'test'
	datawrite.write_ICON_SDE_3rd_order_tfrecord(name = name, eqn_type = 'predator_prey2', 
				all_params = all_params, all_eqn_captions = all_eqn_captions,
			all_ts = all_ts, all_ys = all_ys,alpha=nv_step, repeat=num_repeat)
	...
	# name is 'train' or 'test'
	datawrite.write_ICON_SDE_3rd_order_tfrecord(name = testname, eqn_type = 'predator_prey2', 
				all_params = all_params, all_eqn_captions = all_eqn_captions,
			all_ts = all_ts, all_ys = all_ys,alpha=nv_step, repeat=num_repeat)
