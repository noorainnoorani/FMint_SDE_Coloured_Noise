...
def geombrownian_motion_fn(init, control, ts, dt, params, k, step_fn, key):
    ...
    # Coarse state at selected times: init at t=0, then large_traj at later coarse times
    coarse_first = init[None]                       # shape (1,)
    coarse_rest = large_traj[:-1]                   # shape (L-1,)
    coarse_at_selected = jnp.concatenate([coarse_first, coarse_rest], axis=0)  # (L,)

    # Fine state at selected times: init at t=0, then fine_traj at indices k-1, 2k-1, ...
    fine_indices = jnp.arange(1, L) * k - 1
    fine_rest = fine_traj[fine_indices]
    fine_at_selected = jnp.concatenate([init[None], fine_rest], axis=0)        # (L,)

    errors = fine_at_selected - coarse_at_selected

    return selected_times, coarse_at_selected, large_noise, errors
...
def ornstein_uhlenbeck_fn(init, control, ts, dt, params, k, step_fn, key):
    ...
    coarse_first = init[None]
    coarse_rest = large_traj[:-1]
    coarse_at_selected = jnp.concatenate([coarse_first, coarse_rest], axis=0)

    fine_indices = jnp.arange(1, L) * k - 1
    fine_rest = fine_traj[fine_indices]
    fine_at_selected = jnp.concatenate([init[None], fine_rest], axis=0)
...
# ornstein_uhlenbeck_batch_fn = jax.jit(jax.vmap(ornstein_uhlenbeck_fn, [0,0, None, None, None, None, None, None], (0,0,0)), static_argnums=(-3,-2,))
...
def generate_one_dyn(key, ode_batch_fn, dt, length, num, init_range, params, k, N=1, eqn = "ornstein_uhlenbeck"):
    ...
    return selected_times[..., None], selected_u[..., None], dW[...,None], errors[..., None]
...
if __name__ == "__main__":
    ...
    selected_times, selected_u, dW, errors = generate_one_dyn(key = next(rng), ode_batch_fn = ornstein_uhlenbeck_batch_fn
							, dt=0.05, length = 200, num =1,  init_range = (100,200), params = [theta, mu, sigma]
							,k = 10)
    
    # k = 5

    # selected_times, selected_u, dW, errors = generate_one_dyn(key=next(rng), ode_batch_fn=geombrownian_motion_batch_fn, 
    #                                                           dt=1/252, 
    ...
    selected_times, selected_u, dW, errors = generate_one_dyn(key = next(rng), ode_batch_fn = inhomogeneous_ornsteinuhlenbeck_batch_fn
