...
def expo_decay_fn(init, control,ts,dt, coeff_k ,k, step_fn):
	...
	return selected_times,selected_u
...
expo_decay_batch_fn = jax.jit(jax.vmap(expo_decay_fn, [0,0, None, None, None, None, None], (0,0)), static_argnums=(-2,-1,))
heat_transfer_batch_fn = jax.jit(jax.vmap(heat_transfer_fn, [0,0, None, None, None, None, None], (0,0)), static_argnums=(-2,-1,))
...
def generate_one_dyn(key, ode_batch_fn, dt, length, num,  init_range, params,k):
	...
	selected_times, selected_u = ode_batch_fn(init, control,ts,dt, params ,k, euler_step)
