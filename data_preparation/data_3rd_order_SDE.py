...
def fluxgate_sensor_fn(init, control, ts, dt, params, k, step_fn, key):
	...
	# Align coarse and fine states (first three components) with selected times ts[::k]
	selected_times = ts[::k]
	L = selected_times.shape[0]

	init_stack = jnp.stack([init[0], init[1], init[2]], axis=0)     # (3,)
	coarse_first = init_stack[:, None]                              # (3,1)
	coarse_rest = jnp.stack([large_traj[0][:-1],
	                         large_traj[1][:-1],
	                         large_traj[2][:-1]], axis=0)           # (3,L-1)
	coarse_at_selected = jnp.concatenate([coarse_first, coarse_rest], axis=1)  # (3,L)

	fine_indices = jnp.arange(1, L) * k - 1
	fine_rest = jnp.stack([fine_traj[0][fine_indices],
	                       fine_traj[1][fine_indices],
	                       fine_traj[2][fine_indices]], axis=0)     # (3,L-1)
	fine_first = init_stack[:, None]
	fine_at_selected = jnp.concatenate([fine_first, fine_rest], axis=1)        # (3,L)
	...
	return selected_times, large_states, large_noise_return, errors
...
def stochastic_lorenz_fn(init, control, ts, dt, params, k, step_fn, key):
	...
	init_stack = jnp.stack([init[0], init[1], init[2]], axis=0)
	coarse_first = init_stack[:, None]
	coarse_rest = jnp.stack([large_traj[0][:-1],
	                         large_traj[1][:-1],
	                         large_traj[2][:-1]], axis=0)
	coarse_at_selected = jnp.concatenate([coarse_first, coarse_rest], axis=1)

	fine_indices = jnp.arange(1, L) * k - 1
	fine_rest = jnp.stack([fine_traj[0][fine_indices],
	                       fine_traj[1][fine_indices],
	                       fine_traj[2][fine_indices]], axis=0)
	fine_first = init_stack[:, None]
...
if __name__ == "__main__":
	...
	# selected_times, selected_u, dW, errors = generate_one_dyn(key = next(rng), ode_batch_fn = fluxgate_sensor_batch_fn
	...
	# selected_times, selected_u, dW, errors = generate_one_dyn(key = next(rng), ode_batch_fn = stochastic_lorenz_batch_fn
	...
	# selected_times, selected_u, dW, errors = generate_one_dyn(key = next(rng), ode_batch_fn = predator_prey_batch_fn
	...
	selected_times, selected_u, dW, errors = generate_one_dyn(key = next(rng), ode_batch_fn = predator_prey2_batch_fn
							, dt=0.1, length = 10000, num = 1,  init_range = [(0, 0.8),(0,0.8),(0,0.8)], 
