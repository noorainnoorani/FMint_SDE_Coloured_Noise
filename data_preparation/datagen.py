...
def generate_ode_auto_linear2(seed, eqns, quests, length, dt, num, caption_mode, name):
	'''du/dt = a1 * u + a2 * c + a3
		...
		eqns: number of different equations to generate data for
		quests: number of different inital conditions or control sequences per equation
		length: length of time series data for each simulation
		dt: time step
		num: number of samples to generate per quest
		caption_mode: mode for caption
