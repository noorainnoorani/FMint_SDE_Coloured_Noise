...
def ode_auto_const_fn(init, control, dt, coeff_a, coeff_b, step_fn):
	...
	final, traj = jax.lax.scan(f, init, control) # jax.lax.scan efficiently apply 'step_fn' to each element of 'control'
...
# jax.vmap: vectorize a function over batch dimensions, creating 'batched' version of the ODE solver functions
# ode_autos are batched versions of the ODE solver functions, prepared for JIT compilation
...
if __name__ == "__main__":
	...
	# print('true', jnp.exp(ts))
	# assert jnp.allclose(final, jnp.exp(ts[-1]+dt))
	# assert jnp.allclose(traj, jnp.exp(ts))

	seed = 1
	rng = hk.PRNGSequence(jax.random.PRNGKey(seed))
	ts_expand, control, traj = generate_one_dyn(key = next(rng), ode_batch_fn = ode_auto_const_batch_fn, 
						dt = 0.02, length = 50, num = 100,
