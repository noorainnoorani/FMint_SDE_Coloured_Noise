...
import utils
from absl import app, flags, logging # Abseil, collection of utilities for building Python applications
import plot
...
def run_train():
	...
	time_stamp = datetime.now(pytz.timezone('America/New_York')).strftime("%Y%m%d-%H%M%S")
	stamp = time_stamp
	print("stamp: {}".format(stamp))

	train_warmup_steps = FLAGS.epochs * FLAGS.steps_per_epoch * FLAGS.train_warmup_percent // 100 # 100000


	train_decay_steps = FLAGS.epochs * FLAGS.steps_per_epoch * FLAGS.train_decay_percent // 100 # 1000000
	...
	# data_preparation/data/train* -> train_file_names
	# data_preparation/data/test* -> test_file_names
	...
	if FLAGS.backend == 'jax':
		...
		import jax
		data_num_devices = len(jax.devices()) # 1
	...
	# print('train_file_names: ', train_file_names, flush=True)

	train_data = DataProvider(seed = FLAGS.seed + 1,
							config = train_config,
							file_names = train_file_names, # ["data_preparation/data/train*"],
							batch_size = FLAGS.train_batch_size,#
							shuffle_buffer_size = FLAGS.train_shuffle_buffer_size, # 1000
	...
	for _ in range(FLAGS.epochs * FLAGS.steps_per_epoch + 1):
		# print('train_step: ', runner.train_step, flush=True)
		if FLAGS.tfboard and runner.train_step % (FLAGS.save_freq) == 0:
			logging.info("current time: " + datetime.now(pytz.timezone('America/New_York')).strftime("%Y%m%d-%H%M%S"))
		...
		if FLAGS.profile_level == 0 and ((runner.train_step % FLAGS.loss_freq == 0)
			...
			for dataset in datasets:
				...
				# a icon model is instantiated in runner class
				this_loss = runner.get_loss(data, label)
				this_loss_mean, this_loss_std = np.mean(this_loss), np.std(this_loss)
				print("{} loss: {:.8f}+-{:.8f}".format(dataset.name, this_loss_mean, this_loss_std), flush=True)
				if FLAGS.tfboard:
			...
			for demo_num, caption_id, caption, data in split_data(caption, data, test_demo_num_list, test_caption_id_list):
				...
				if caption_id == -1:
					...
					print("test with demo num {}, no caption  , error: {:.8f}+-{:.8f}".format(
				...
				# with caption
					...
					this_error_mean, this_error_std = np.mean(this_error), np.std(this_error)
					print("test with demo num {}, caption id {}, error: {:.8f}+-{:.8f}".format(
...
if __name__ == '__main__':
	...
	flags.DEFINE_integer('profile_level', 0, '0: usual training, 1: profile training,: profile without loading data')
	...
	flags.DEFINE_integer('restore_step', 600000, 'restore step')
